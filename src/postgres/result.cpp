...
int result::rows() const { return res_ ? PQntuples(res_) : 0; }

int result::columns() const { return res_ ? PQnfields(res_) : 0; }

std::string_view result::column_name(int col) const { return res_ ? PQfname(res_, col) : ""; }

bool result::is_null(int row, int col) const { return res_ ? (PQgetisnull(res_, row, col) == 1) : true; }
